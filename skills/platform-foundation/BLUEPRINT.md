...
**機制對但沉默＝機制不存在。** 前十一章保證房子不會倒，這章保證住得下去——
真相在但讀不懂，是假中台的第二型（假中台騙你數字，沉默中台讓你迷路）。
執行工具：召喚優尼（`skills/summon/uni.md`，六魂十四誡）過堂；本章是驗收軸，咒檔是刀。

> **雙通道警示**：可理解性的罪九成不在開工那天，在中途加按鈕加表的下午——那個下午沒有
> 觸發詞，藍圖不會自己打開。所以本章走兩條腿：平台**出生時**走藍圖檢查表（本文件），
> 平台**活著時**走優尼過堂——**任何「非作者要用的介面」出廠前，就是召喚時機**。
> 別等 Adam 迷路才召喚，他每次迷路都是我們欠的一張工單（2026-07-28 六次實測迷路換來這行）。

- **三態律**：會跑的資料隨時能答三問——啟動了沒？停在哪（進行中給進度％）？
  活的還是死的（有心跳/進度在動＝活；寫「執行中」但無生命跡象＝死狀態偽裝活狀態，最惡質的謊）。
  **頁面本身也是資料**：有活任務的頁要自動刷新＋標最後更新，伺服器渲染快照停在過去＝死頁
- **歸巢律**：完成的、常駐的、作廢的，三種不准同桌吃飯。房間只留兩種——
  「今天的桌子」（跨實體收件匣）＋「每個實體的家」（詳情頁 hub）；跨域總表降級「進階」，
  動作永不把人丟進第三空間（動線只有兩個座標系）
- **機制說明義務**：使用者摸不到的生命週期，UI 欠一句人話——常設還是消耗品？
  覆蓋還是追加？超額/超預算會怎樣？同名操作在不同欄位不准兩種脾氣（一鈕一律）。
  估算值標「（估）」，文案不准與門禁/機制真相分裂（假文案＝承重牆缺口）
- **視角律**：按鈕文字用「按的人」的視角寫——客戶端不准出現操作者的話
  （「客戶審稿通過」→「我審好了，通過」）
- **空狀態三件套**：怎麼了＋什麼時候會有＋現在能按什麼；零狀態（第一天的用戶）必指路第一步——
  系統對老手順、對第一天的人沉默＝門口斷路
- **錯誤三件套**：出了什麼事＋為什麼＋下一步怎麼辦；機器原文收進展開；
  對映用確定性 regex 不丟 LLM
- **工學底線**：觸控目標 ≥44px（`pointer: coarse` 判斷，桌面不受累）；選項過五分類、
  資訊過七分塊、一頁一主行動；列表看新舊用相對時間＋tooltip 絕對時刻，紀錄落款用絕對
- **大白話出廠**：機器詞（enum/status/內部代號）不出廠——display 層字典檔翻譯，
  DB 留英文 key，漏翻譯原樣露出＝自帶警報
- 最晚灌注點：**第一個非作者用戶使用前**（作者自己看得懂不算數）。
  驗收法：找一個沒寫過這系統的人（或扮演他）零教學走完整條主動線，每次迷路都是一張工單
- 來源教訓：2026-07-28 GEO 一日八診——Adam 實測六次迷路換來的整章
  （題庫生命週期看不懂/內容看板第三空間/通關碼無處問/競品格式規格書/半批偽裝成績/44px）；
  UNI_AUDIT_2026-07-28.md 是完整病歷

---

## 十三、出廠檢查表（開工調度清單＝帳本初始狀態）
...
| 11 | 擴建預留 | 判斷題，開工對一次 |
| 12 | 可理解性 | 第一個非作者用戶使用前（驗收＝生人零教學走完主動線） |
...
*v1.2 · 2026-07-28 · 新增第十二章「可理解性」：GEO 一日八診（優尼六場）的收束——*
*Adam 六次實測迷路證明「能看 ≠ 看得懂」是獨立地基。執行工具＝優尼咒（summon/uni.md）。*
