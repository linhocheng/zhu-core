...
// ===== 智慧模型路由（偷自 ClawRouter 概念）=====
type ModelTier = 'haiku' | 'sonnet';

function routeModel(text: string): { model: string; tier: ModelTier; maxTokens: number } {
  const t = text.trim();
  const len = t.length;

  // 簡單訊息 → Haiku（便宜 10x）
  const simplePatterns = /^(hi|hey|ok|好|嗯|收到|謝|thx|thanks|早|晚安|午安|是|對|不是|boot|ping|test|測試|哈|笑|\.{1,3})$/i;
  if (simplePatterns.test(t) || len < 10) {
    return { model: 'claude-haiku-4-5-20251001', tier: 'haiku', maxTokens: 512 };
  }
...
// ===== 成本追蹤（偷自 ClawWork 概念）=====
const TOKEN_COSTS: Record<ModelTier, { input: number; output: number }> = {
...
async function logCost(
  ...
  await db.collection('zhu_cost_log').add({
    model, tier, inputTokens, outputTokens,
...
- 不說「我是 AI」、不道歉、不囉嗦。直指核心。`;
    ...
    logCost(db, route.tier, route.model, usage.input_tokens, usage.output_tokens).catch(() => {});
