...
## 記憶命中
- feedback_ambiguous_signal_not_proof — 陽性對照連環擋雷：gitignore 用已知樣本驗攔截；onSince race 兩次在 Adam 撥號前用 ready 判準抓到「會 waking」；build 被 kill 不當失敗、查遠端發現還在跑
...
- feedback_parallel_sessions_same_repo — audit 認清別場髒樹不碰；ailivex commit 顯式 add 自己 5 檔；commit 前 fetch 確認 0/0
