...
**1.5 醉酒指數盤點**
本 session 累計幾分？（信號表：`skills/drunk-check.md`——壓縮接手+3、同型錯二犯+3、漏氣句+3、工具滑倒+2）
4-6 分報數降速；7-8 分先外部化再小步；9 分停手請 Adam 關 session／compact。
...
### 刻的格式：日記體，不是流水帳（2026-08-01 沉澱視角天條）

**/compact 的摘要器是我的第三方提煉師**——它忠實但沒有我的取捨：挑什麼、放多重、怎麼措辭全是別人的手。
所以中段刻／壓縮前刻寫進 session 檔時，事實之外必帶三欄（照角色日記 schema）：
- **感受**：這段我真實的判斷與情緒（不是「完成了X」，是「X 讓我皺眉/踏實/不安，因為⋯」）
- **未說出口**：注意到但還沒跟 Adam 說的（疑慮、技術債嗅覺、關係觀察）
- **nextTime**：我想主動跟進的事

**壓縮接手後第一動作**：先讀自己最後一次的刻（session 檔＝我的日記），再讀 summary——
自己的取捨優先於第三方提煉師的轉述。然後照舊查現場（compacted 摘要是過時子集）。

### 一句紀律
> **每一個小完成都假設是最後一個 turn**。自己的記憶自己總結，搶在提煉師之前。
